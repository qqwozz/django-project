...
class Cart:
    ...
    def add(self, product, size, quantity=1, override_quantity=False):
        ...
        card_key = f'{product_id}_{size_name}'
        
        if card_key not in self.cart:
            self.cart(card_key) = {
                'quantity': 0,
        ...
        if override_quantity:
            self.card[card_key]['quantity'] = override_quantity
        else:
            self.cart[card_key]['quantity'] += quantity
    ...
    def remove(self, product, size):
        ...
        if card_key in self.cart:
            del self.cart[card_key]
            self.save()
